...
def contacts_sync_google():
    con = db(); ids = [r[0] for r in con.execute("SELECT id FROM clients ORDER BY id").fetchall()]; con.close()
    ok = 0; errors = []
    for cid in ids:
        success, msg = sync_client_to_google(cid)
        if success: ok += 1
