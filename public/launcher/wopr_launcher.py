...
WINDOW_ICON_ICO=PUBLIC_DIR/"static"/"WOPR.ico"
WINDOW_ICON_PNG=PUBLIC_DIR/"static"/"favicon.png"
    ...
    def __init__(self):
        ...
        try:
            if IS_WINDOWS and WINDOW_ICON_ICO.is_file():
                self.iconbitmap(default=str(WINDOW_ICON_ICO))
            elif WINDOW_ICON_PNG.is_file():
                self._wopr_window_icon = tk.PhotoImage(file=str(WINDOW_ICON_PNG))
                self.iconphoto(True, self._wopr_window_icon)
        except Exception as exc:
            log(f"Icône launcher non chargée : {exc}")
